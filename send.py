...
ap.add_argument('--group', required=True)          # 239.1.1.1 (multicast group)
ap.add_argument('--iface', required=True)          # 10.0.1.x (source interface IP)
...
def build_px(group_ip_str: str, seq: int) -> bytes:
    # PX = group_ip (32 bits) + seq (16 bits) in network byte order
    return struct.pack("!4sH", socket.inet_aton(group_ip_str), seq)

def send_one(seq: int):
    payload = build_px(args.group, seq)
    tx.sendto(payload, (args.group, args.port))
    print(f"[leader {args.iface}] sent REQ seq={seq} (group={args.group}) -> {args.group}:{args.port}")
...
def listen_acks():
    ...
    while True:
        try:
            ...
            # Expect ACK payload to start with PX too: group_ip + seq (6 bytes)
            if len(data) >= 6:
                g_raw, ack_seq = struct.unpack("!4sH", data[:6])
                g = socket.inet_ntoa(g_raw)
                rest = data[6:]
                print(f"[leader] ACK from {addr}: group={g}, seq={ack_seq}, rest={rest!r}")
            else:
