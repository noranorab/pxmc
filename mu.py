...
import socket
from scapy.all import Ether, IP, UDP, sendp, Raw, send, get_if_addr, get_if_hwaddr
...
class MuReplica(asyncio.DatagramProtocol):
    ...
    def sendto(self, addr, msg):
        ...
        if msg["t"] == "PERM_REQ":
            ...
            print("Replica sending PERM_REQ")
        elif msg["t"] == "PERM_ACK":
            dport = 5004
            dst_mac = ip_to_mac(addr[0])
            print("Replica sending PERM_ACK")
        else:
            dport = 5000
            dst_mac = ip_to_mac(addr[0])
            ...
            while True:
                ...
                if req_id is not None:
                    ...
                    if reply_addr is not None:
                        self.sendto(reply_addr, {"t": "PERM_ACK", "from": self.id, "ballot": ballot, "req_id": req_id})
...
#        self.sock.setblocking(False)
    ...
    async def propose(self, myValue: Any):
        ...
        while not done:
            ...
            self.fast_path = False
            for p in confirmed + [self.id]:
                await self.write_FUO(p, self.myFUO)
